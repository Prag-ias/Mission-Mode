...
### D44 — A provably wrong key loses to the arithmetic
**Context:** CSAT 2025 Q59 is a work-rate problem. The supplied key says (d) 6; the rates as printed give n = 4 exactly, and 6 leaves Y one day of work rather than the stated 8⅔. The reconciliation pass refused to defend the key, and the key image was confirmed correctly transcribed.
**Decision:** For a question with a *provable* answer, a demonstrably wrong key does not win. The derived answer is stored (`answer_source = 'derived'`), `disputed = true` badges it and excludes it from accuracy stats, and the explanation shows the full working and states plainly that the supplied key disagrees. This is the narrow exception to "official key wins": it applies only where the answer is computable from the question alone, never to comprehension or judgement items — there the key wins even when a solver disagrees (2021 Q39, and every reconciled comprehension item).
**Consequence:** The bank never teaches an answer that the question's own numbers contradict, and the two questions where the source key is unusable are visible rather than silently absorbed.

### D45 — Mains mode is deferred until after Prelims
**Context:** Sociology sits in the app as a Prelims-shaped subject — 28 topics, read → revision ladder, `pyq_drills = false` — but it is never tested in Prelims. It is 500 Mains marks, examined by writing 150–250 word answers in 8–11 minutes, and 131 hours of it are already scheduled before 23 May 2027. A `/mains` mode was scoped on 1 Sep 2026: app-picked PYQ, timer, photograph of the handwritten sheet, structural self-check, coverage across the 28 topics, two new tables. Estimated ~8–9 hours.
**Decision:** Not building it. Pragaman chose to keep every remaining build hour on Prelims and revisit Mains after 23 May 2027. The scoping stands and does not need redoing.
**Consequence:** The modelling gap is accepted, knowingly: until Mains mode exists, Saturday Sociology is tracked as reading and recall, and answer-writing practice lives outside the app. Two things must be settled before any future build — (1) the **1 Feb 2027 feature freeze**, which Mains mode would violate and which only he can amend; (2) whether scope is Sociology alone or GS and Essay too. What it needs from him is unchanged: Sociology Mains PYQ PDFs (2016+) in `PYQs/Mains/Sociology/`, his handwriting speed, and any model-answer source. P6, the `--mains` planner flag, and the `ANSWER_WRITING`/`MAINS_GS`/`ESSAY` block kinds already exist and were left untouched.
